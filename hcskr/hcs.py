...
def userlogin(
    name, birth, area, schoolname, level, password, loop=asyncio.get_event_loop()
):
    return loop.run_until_complete(
        asyncUserLogin(name, birth, area, schoolname, level, password)
    )
...
async def asyncSelfCheck(
    name, birth, area, schoolname, level, password, customloginname=None
):
